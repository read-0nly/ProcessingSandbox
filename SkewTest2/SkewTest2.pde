...
void draw(){
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      float r = red(img.pixels[int(abs(locI+buffer/1.3)%img.pixels.length)]);
      float g = green(img.pixels[int(abs(locI+(buffer/1.5))%img.pixels.length)]);
      float b = blue(img.pixels[int(abs(locI+buffer/1.4)%img.pixels.length)]);
